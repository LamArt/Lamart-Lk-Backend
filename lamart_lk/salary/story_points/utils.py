        ...
        try:
            for issue in issue_data.get('issues', []):
                ...
                for i in range(time_delta):
                    if period == 'w':
                        start_time = (self.current_date - timedelta(days=self.current_date.weekday()) - timedelta(
                            weeks=i)).replace(hour=0, minute=0, second=0, microsecond=0)
                        end_time = start_time + timedelta(days=6, hours=23, minutes=59, seconds=59)
                    else:
                        start_time = (self.current_date - timedelta(days=self.current_date.day - 1)).replace(
                            day=1) - timedelta(
                            weeks=i * 4)
